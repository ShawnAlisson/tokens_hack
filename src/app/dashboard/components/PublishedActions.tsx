...
export default function PublishedActions({ refreshTrigger }: { refreshTrigger: number }) {
  ...
  return (
    <div className="glass-panel p-6 rounded-2xl h-full flex flex-col justify-between">
      <div>
        ...
        <p className="text-xs text-slate-400 mt-0.5">Live briefs synchronized to your Notion workspace (F)</p>
        ...
        <div className="mt-4 space-y-3 max-h-[350px] overflow-y-auto pr-1">
          {actions.length > 0 ? (
            actions.map((act) => (
              <div 
                key={act.id} 
                className="glass-panel-glow-emerald p-4 rounded-xl border border-emerald-500/20 hover:border-emerald-500/40 transition-all space-y-2 relative"
              >
