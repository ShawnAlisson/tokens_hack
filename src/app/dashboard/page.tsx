...
import BrandProductsPanel from "./components/BrandProductsPanel";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (wasOnboarded) loadTenant();
    fetch("/api/dashboard/tenants").then((r) => r.json()).then((d) => setAllTenants(d.tenants || []));
  }, []);
  ...
  const switchTenant = async (tenantId: string) => {
    ...
    localStorage.setItem("brandcompete_tenant", tenantId);
    ...
    <main className="min-h-screen p-4 md:p-6 lg:p-8 max-w-[1400px] mx-auto space-y-5">
      ...
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            >
              {tenant?.logo_url && (
                <img src={tenant.logo_url} alt="" className="w-5 h-5 object-contain" />
              )}
                ...
                {allTenants.map((t) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-sm hover:bg-slate-50 flex items-center gap-2 ${
                      tenant?.id === t.id ? "text-teal-700 font-semibold bg-teal-50" : "text-slate-600"
                    }`}
                  >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
            ...
            {activeBrief && parsedBrief ? (
              <div className="border border-emerald-200 bg-emerald-50/50 rounded-xl p-5 space-y-4 animate-fade-in">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] bg-emerald-600 text-white font-bold px-2.5 py-1 rounded-md uppercase tracking-wider">
                    Generated Brief
                  </span>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-emerald-200">
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {activeBrief.brand_facts_used?.map((fact, i) => (
                      <span key={i} className="text-[9px] font-mono bg-teal-50 text-teal-700 border border-teal-200 px-2 py-0.5 rounded-md">
                        {fact}
                      </span>
                    ))}
                  </div>
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white font-bold text-xs rounded-xl flex items-center gap-1.5 transition-colors"
                  >
              ...
              <div className="border-2 border-dashed border-slate-200 rounded-xl py-14 flex flex-col items-center text-center px-6">
                <div className="w-14 h-14 rounded-2xl bg-slate-100 flex items-center justify-center mb-3">
                  <FileText className="w-7 h-7 text-slate-400" />
                </div>
                <p className="text-sm font-semibold text-slate-600">No campaign selected</p>
                <p className="text-xs text-slate-400 mt-1 max-w-sm">
                  Click <strong>Run Strike</strong> on a threat in the intelligence stream, or select a published campaign above.
                </p>
              </div>
            )}
          </div>
        ...
        <aside className="lg:col-span-5 space-y-5">
          <div className="bc-panel rounded-2xl p-5 space-y-4">
            <div>
              <h2 className="text-base font-bold text-slate-800" style={{ fontFamily: "var(--font-heading)" }}>
                Brand Context
              </h2>
              <p className="text-xs text-slate-500">Grounded positioning from your brand knowledge base</p>
            </div>
            <BrandInsightPanel refreshTrigger={refreshTrigger} />
            <BrandProductsPanel refreshTrigger={refreshTrigger} />
            <CompetitorWatchlist
              refreshTrigger={refreshTrigger}
